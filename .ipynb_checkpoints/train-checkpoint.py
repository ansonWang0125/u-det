...
def get_callbacks(arguments):
    ...
    csv_logger = CSVLogger(join(arguments.log_dir, f"{arguments.output_name}_log_{arguments.time}.csv"), separator=',')
    tb = TensorBoard(arguments.tf_log_dir, histogram_freq=0)
    model_checkpoint = ModelCheckpoint(join(arguments.check_dir, f"{arguments.output_name}_model_{arguments.time}.hdf5"),
